...
### 新功能
- 新增图片筛选：支持快速找到包含图片的 Memos（包括正文图片和附件图片）
- 任务筛选支持三态切换：点击可依次切换为「未完成任务」→「全部任务」→ 取消筛选
- 筛选项为 0 时自动隐藏对应标签，界面更简洁
- 筛选标签支持点击激活/取消，激活时高亮显示

### 优化
- 统一图片预览体验：正文图片和附件图片现在共享同一个预览画廊，可以左右滑动浏览全部图片
- 附件图片显示样式优化，与正文图片保持一致
- 连续多张图片（单换行分隔）自动合并为一行显示，空行分隔才会换行
- 统一侧边栏和筛选栏的图标样式
- 筛选栏显示文本支持多语言
- 使用内置 UI 组件替代第三方组件库，避免依赖问题

### 文档
- README 新增 MCP Server 介绍
- README 新增版本说明
- 更新迁移兼容说明
...
### 新功能
- 编辑器：编辑备忘录时新增保留更新时间的开关
- 回顾：备忘录卡片新增编辑按钮，支持静默刷新

### 修复
- 备忘录卡片：修复折叠与内部代码块展开冲突的问题
- 回顾：按钮添加禁用状态和加载动画
- 回顾：使用本地化日期格式，加载时保持头部可见
